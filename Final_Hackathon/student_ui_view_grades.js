...
import { id } from "./var.js" // NEED TO BE EDITED
...
try {
  ...
  async function LoadDataAdvance() {
    var ref = doc(db, "STUDENT_LIST", "STUDENT_DATA", id, selectedVariable);
  ...
  async function subject01() {
    var ref = doc(db, "CLASS_RECORD", id, data1 + selectedVariable, selectedVariable);
    const docsnap = await getDoc(ref);
  ...
  async function subject02() {
    var ref = doc(db, "CLASS_RECORD", id, data2 + selectedVariable, selectedVariable);
    const docsnap = await getDoc(ref);
    if (docsnap.exists()) {
      //assigning value from firestore to table data
  ...
  async function subject03() {
    var ref = doc(db, "CLASS_RECORD", id, data3 + selectedVariable, selectedVariable);
    const docsnap = await getDoc(ref);
    if (docsnap.exists()) {
      //assigning value from firestore to table data
  ...
  async function subject04() {
    var ref = doc(db, "CLASS_RECORD", id, data4 + selectedVariable, selectedVariable);
    const docsnap = await getDoc(ref);
  ...
  async function subject05() {
    var ref = doc(db, "CLASS_RECORD", id, data5 + selectedVariable, selectedVariable);
    const docsnap = await getDoc(ref);
  ...
  async function subject06() {
    var ref = doc(db, "CLASS_RECORD", id, data6 + selectedVariable, selectedVariable);
    const docsnap = await getDoc(ref);
    if (docsnap.exists()) {
      //assigning value from firestore to table data
